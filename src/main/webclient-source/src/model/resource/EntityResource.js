...
function toJsonSchema(desc, constraints = []) {
  ...
  Object.keys(colCfgMap).forEach((colKey) => {
    const colId = colCfgMap[colKey].colId;
    const colName = colCfgMap[colKey].colName;
    const dataJdbcType = colCfgMap[colKey].dataJdbcType;
    const dataLength = colCfgMap[colKey].dataLength;
    const dataLengthMin = colCfgMap[colKey].dataLengthMin;
    const dataLengthMax = colCfgMap[colKey].dataLengthMax;
    // 排除不可用项目
    const disabledKey = disableColSet.find(d => d === colId);
    // 排除隐藏项目
    const hideKey = hideColSet.find(h => h === colId);
