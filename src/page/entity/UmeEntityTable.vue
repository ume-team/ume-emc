    ...
    <ume-table border stripe class="ume-entity-table" :data="data" ref="entityTable">
      <ume-table-column
        ...
        <template slot-scope="scope">
          <span v-if="typeof scope.row[col.colId] === 'object'">
            {{ scope.row[col.colId].label }}
          </span>
      ...
      <ume-table-column fixed="right" label="操作" :width="controlColumnWidth">
        <template slot-scope="scope">
          <div class="control-button-container">
            <ume-button type="text" @click="doUpdate(scope.row)"
              v-if="isShowUpdateButton">修改</ume-button>
            <span class="button-separator" v-if="isShowUpdateButton && isShowDeleteButton">|</span>
            <ume-button type="text" @click="doDelete(scope.row)"
              v-if="isShowDeleteButton">删除</ume-button>
          </div>
        </template>
      </ume-table-column>
    </ume-table>
