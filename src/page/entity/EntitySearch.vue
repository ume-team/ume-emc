...
export default {
  ...
  methods: {
    doFetch() {
      ...
      const getEmDataList = EntityResource.getEmDataWithConstraintsList(
        this.entityId,
        this.searchCondition,
      );
      const getEmDataCount = EntityResource.getEmDataCount(
        this.entityId,
        this.searchCondition,
      );
      Promise.all([getEmDesc, getEmConstraint, getEmDataList, getEmDataCount])
        .then((res) => {
          this.$set(this, 'entityDesc', res[0]);
          this.$set(this, 'entityConstraint', res[1]);
          this.$set(this, 'entityData', res[2]);
          this.$set(this, 'totalCount', res[3]);
        });
    },
    doSearch() {
      this.currentPage = 1;
